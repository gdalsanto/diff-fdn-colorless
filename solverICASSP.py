...
def main(args, train_dataset, valid_dataset):
    ...
    param = scipy.io.loadmat('/Users/dalsag1/Dropbox (Aalto)/aalto/projects/fddsp/output/20240828-154606/parameters_init.mat')
    # overwrite the initial parameter values
    net.A = torch.nn.Parameter(torch.tensor(param['A']).reshape(net.A.shape), requires_grad=True)
    net.B = torch.nn.Parameter(torch.tensor(param['B']).reshape(net.B.shape), requires_grad=True)
    net.C = torch.nn.Parameter(torch.tensor(param['C']).reshape(net.C.shape), requires_grad=True)
    # construct trainer4
