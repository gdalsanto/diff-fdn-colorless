function [drivenRes,directTerm,undrivenResidues,matrixRes] = dss2res_mod(poles,loop,B,C,D)
...
matrixRes = zeros(numberOfPoles,N,N);
for it = 1:numberOfPoles
    pole = poles(it);
    b = B.at(pole);
    c = C.at(pole);
    ...
    matrixRes(it,:,:) = adjugate(l);
end
matrixRes = matrixRes ./ r_denominator ;
