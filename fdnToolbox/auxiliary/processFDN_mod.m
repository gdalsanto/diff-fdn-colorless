function output = processFDN_mod(input, delays, A, B, C, D, varargin)
...
while blockStart < inputLen
    ...
    block = input(blkInd,:);
    %% TO DO: 
    % FeedbackMatrix.filter 
    % DelayFiltersSmall.filter 
    % DelayFilter.filter 
    % scattering matrix with attenuation ? 
    % input and output gains maybe 
    % the Filter matrix can be generated by using the zFIR (check
    % example_scatteringFDN.m)
    
    % Get main delays outputs
    delayOutput = DelayFilters.getValues(blkSz);
    if ~isempty(absorptionFilters)
       delayOutput = absorptionFilters.filter(delayOutput); 
    end

    % Get Feedback Matrices output 
    feedbackMatrixInput = delayOutput + block;
    feedback = FeedbackMatrix.filter(feedbackMatrixInput);
    if ~isempty(extraMatrix)
        feedback = extraMatrix.filter(feedback);
    end
    

    % update delays internals states
    DelayFilters.setValues(feedback);

    % get output 
    output(blkInd,:) = OutputGains.filter(feedback);
